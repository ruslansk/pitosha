...
def generate_summary(code_object):
	...
	def generate_decoration(key, value, index):
		if key == 'co_flags':
			...
			return html_format('%(value)s: %(ss)s' % {'value': value, 'ss': ', '.join(flag_name_list)})
			#sa = ', '
			#ss = sa.join(flag_name_list)
			#return html_format('%(value): %(ss)' % vars())
			#return html_format('{0}: {1}'.format(value, ', '.join(flag_name_list)))
		elif key == 'co_consts':
			if hasattr(value, 'co_code'):
				return '<a href="%(a)s">%(a)s</a>' % {'a': value.co_name}
			else:
				return html_format('%(0)s: %(1)s' % {'0': repr(value), '1': type(value)})
		elif key == 'co_varnames':
			if index < code_object.co_argcount:
				return html_format('%(0)s (pos arg: %(1)s)' % {'0': repr(value), '1':index})
			elif varargs and index == code_object.co_argcount + varargs - 1:
				return html_format('%(0)s (excess pos arg (*args))' % {'0': repr(value)})
			elif varkwargs and index == code_object.co_argcount + varargs + varkwargs - 1:
				return html_format('%(0)s (excess key arg (**kwargs))' % {'0': repr(value)})
...
#
#	def (self, code_object, code_object_path):
...
c = load_code_object('utils.pyc')
a = generate_summary(c)
b = generate_disassembly(c)
print 'ok'
